...
def wordNet(train_text, label, dataset_name=''):
    ...
    train_data.to_csv('{}_wordnet_augment.csv'.format(
        dataset_name), index=False)
...
def Contextual(train_text, label, dataset_name=''):
    ...
    train_data["text1"] = pd.Series(auglist1)
    train_data["text2"] = pd.Series(auglist2)
    train_data.to_csv('{}_contextual_augment.csv'.format(
        dataset_name), index=False)
...
def word_deletion(train_text, label, dataset_name=''):
    ...
    train_data.to_csv('{}_wordDelete_augment.csv'.format(
        dataset_name), index=False)
...
def back_translation(train_text, label, dataset_name=''):
    ...
    for i, text in tqdm(enumerate(train_text)):
        ...
        if(((i+1) % batch_size == 0) & (i != 0)):
            ...
            time.sleep(2)
            final_result1 = get_trans(
                get_translation_text(target_result1), 'fra', 'en')
            time.sleep(2)
            final_result2 = get_trans(
                get_translation_text(target_result2), 'zh', 'en')
            time.sleep(2)
    ...
    train_data["text1"] = pd.Series(auglist1)
    train_data["text2"] = pd.Series(auglist2)
    train_data.to_csv('{}_translation_augment.csv'.format(
        dataset_name), index=False)
