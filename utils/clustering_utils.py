...
def get_cluster_prob(embedding, cluster_center, args: argparse.ArgumentParser):
    """get the probability that each point belongs to each cluster
    ...
    Returns:
        origin_distance.min(1)[0]: The shortest distance from each cluster center
        prob: the probability that each point belongs to each cluster
    """
...
def get_centers_distance(cluster_center):
    """get the distance between cluster centers
    
    Args：
        cluser_center: Cluster center
        
    Returns:
        distance.sum()/2: the distance between cluster centers
...
def get_auxiliary_distribution(cluster_prob):
    ...
    Args: 
        cluster_prob: the probability that each point belongs to each cluster
        
    Returns:
        auxiliary_distribution: pjk
