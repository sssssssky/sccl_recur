...
def get_kmean_center(model, dataloader: DataLoader, args: argparse.ArgumentParser):
    ...
    for i, batch in enumerate(dataloader):
        ...
        embedding_text = model.encode(text, show_progress_bar = False, convert_to_tensor = True).cpu().numpy()
        if i!=0:
            all_embedding_text = np.concatenate((all_embedding_text, embedding_text), axis = 0)
...
def get_cluster_prob(embedding, cluster_center, args: argparse.ArgumentParser):
    ...
    """
    origin_distance = torch.sum((embedding.unsqueeze(1) - cluster_center)**2, dim = 2)
