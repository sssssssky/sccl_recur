...
class sccl(nn.Module):
    def __init__(self, argsss: argparse.ArgumentParser):
        ...
        initial_cluster_center = torch.tensor(
            get_kmean_center(self.embedding_model, self.dataloader,self.args)[0],dtype = torch.float , requires_grad = True
        )
        
        self.cluster_center = nn.Parameter(initial_cluster_center)


    def char2id(self, batch: dict):
    ...
    def forward(self, batch):
        ...
        embedding0, embedding1, embedding2 = self.id2embedding(ids[0]), self.id2embedding(ids[1]), self.id2embedding(ids[2])
        ...
        # clusterring_loss
        _, cluster_prob = get_cluster_prob(embedding0, self.cluster_center, self.args)
        _, cluster_prob1 = get_cluster_prob(embedding0, self.cluster_center, self.args)
        _, cluster_prob2 = get_cluster_prob(embedding0, self.cluster_center, self.args)
        
        target = get_auxiliary_distribution(cluster_prob).detach()
        #clustering_loss = self.cluster_loss((cluster_prob + 1e-08).log(),target) / cluster_prob.shape[0]
        ...
        #return feat1,feat2,cluster_prob,target
        return return_dic
